...
class LaneChangeDecider():
    ...
    def get_current_observation(self):
        ...
        #     return
        df_rows = []
        for obj_type in self.obs_list:
            for obj_name, obj_info in self.obs_list[obj_type].items():
                obj_x = obj_info.x
                obj_y = obj_info.y
                if 0 <= obj_y:
                    local_x, local_y = obj_x, obj_y
        ...
        self.obs_df = pd.DataFrame(df_rows)  # 直接转换为 DataFrame 存储
    
    def publish_new_refline(self):
        data_cone = []
        if len(self.obs_df) != 0:
            data_cone = self.obs_df[self.obs_df['type'] == 'cone']
            data_pedestrain = self.obs_df[self.obs_df['type'] == 'pedestrain']
            data_vehicle = self.obs_df[self.obs_df['type'] == 'vehicle']
            data_bycicle = self.obs_df[self.obs_df['type'] == 'bycicle']
        if len(data_cone) == 0:
            ...
            data_cone["utm_coords"] = data_cone.apply(lambda row: self.localxy2utm([row['local_x'], row['local_y'], row['yaw'], row['length']], self.state), axis=1)
            ...
            data_cone["utm_length"] = data_cone["utm_coords"].apply(lambda x: x[3])
            # 画出utm坐标系下的cone
